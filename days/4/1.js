...
const findBestGuard = guardStats => {
  return Object.keys(guardStats).reduce((acc, k) => {
    const { max, sum, maxIndex } = guardStats[k].reduce((stats, o, i) => {
      ...
      const lsum = o + stats.sum;
      return { max: lmax, sum: lsum, maxIndex: lmaxIndex };
    }, { max: 0, sum: 0, maxIndex: 0});
    if (sum > acc.mostHours) {
      return { guard: parseInteger(k), mostHours: sum, bestMinute: maxIndex };
    }
    return acc;
  }, { mostHours: -Infinity, bestMinute: -Infinity, guard: -Infinity });
}
