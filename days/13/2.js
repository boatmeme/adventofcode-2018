...
class Track {
  constructor(arr2d) {
    ...
    this.lastRemaining = false;
  }
  tick() {
    this.ticks++;
    const piecesToMove = this.pieces.reduce((acc, row) => [...acc, ...row.filter(p => p.cart !== null)], []);
    if (piecesToMove.length === 1) {
      this.lastRemaining = piecesToMove[0];
      return false;
    }
    for(let j = 0; j < piecesToMove.length; j++) {
      ...
      if (targetPiece.collision) {
        targetPiece.removeCart();
        targetPiece.collision = false;
...
(async () => {
  ...
  //console.log(track.toString());
  console.log(`Ticks: ${track.ticks}\n`);
  console.log(`Last Remaining: ${track.lastRemaining.x},${track.lastRemaining.y}\n`);
